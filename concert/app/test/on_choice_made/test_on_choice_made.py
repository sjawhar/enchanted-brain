...
def get_event(choice="COLOR_BLUE", choice_type="CHOICE_COLOR", timestamp="2017-05-14T20:20:03.000Z", user_id="userId"):
    return {
            "body": json.dumps(
                {
                    "data": (
...
def test_choices(choice_type, choice, timestamp, user_id):
    event = get_event(user_id=user_id, choice=choice, choice_type=choice_type, timestamp=timestamp)
    ...
    with Stubber(sns) as sns_stub, Stubber(firehose) as firehose_stub:
        sns_stub.add_response(
            "publish", SNS_SUCCESS_RESPONSE, sns_expected_params
        )
